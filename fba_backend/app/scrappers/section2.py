...
# API keys (kept in file as requested)
...
def get_company_symbol(company_name):
    ...
    try:
        ...
        data = response.json()
        print(f"Symbol search response for {company_name}: {data}")
...
def fetch_fmp_data(endpoint, company_symbol):
    ...
    try:
        ...
        print(f'Data from {endpoint} for {company_symbol}:', data)
        return data
    except Exception as e:
...
def main(company_name):
    ...
    return results if results else {"message": "No relevant data found."}
